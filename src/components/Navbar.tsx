  ...
  return (
    ...
    >
      <div className="container mx-auto flex items-center justify-between py-3 md:py-4 px-4">
        <Link to="/" className="relative z-10 flex items-center gap-2.5 md:gap-3 group">
          <img
            src={logo}
            alt="Kawsay Piscinas y Jacuzzis"
            className={`transition-all duration-500 ease-in-out ${scrolled ? "h-10 md:h-12" : "h-12 md:h-16"
              } md:group-hover:scale-105`}
          />
          <span className={`font-display font-bold text-xl md:text-2xl tracking-tight transition-all duration-300 ${scrolled ? "text-primary" : "text-primary-foreground"
            }`}>
            KAWSAY
          </span>
        </Link>
          ...
          {links.map((l) => {
            ...
            return (
              <li key={l.href}>
                <Link
                  to={l.href}
                  className={`relative font-body text-sm font-medium px-4 py-2 rounded-lg transition-all duration-200 ${scrolled
                      ? active
        ...
        <button
          ...
          className={`md:hidden relative z-10 p-2 rounded-lg transition-colors ${scrolled ? "text-foreground" : "text-primary-foreground"
            }`}
        >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`md:hidden backdrop-blur-xl border-t border-border/10 overflow-hidden transition-all duration-300 ${scrolled ? "bg-card/95" : "bg-black/60"
              }`}
          >
              ...
              {links.map((l) => (
                <li key={l.href}>
                  <Link
                    ...
                    className={`block font-body text-sm font-medium px-4 py-3 rounded-lg transition-all duration-200 ${location.pathname === l.href
                        ? "text-primary bg-secondary"
                        : scrolled
